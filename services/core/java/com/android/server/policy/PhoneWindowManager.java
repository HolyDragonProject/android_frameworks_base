...
public class PhoneWindowManager implements WindowManagerPolicy, DeviceKeyHandler.IDeviceKeyHandlerCallback {
    ...
    private static final int MSG_SYSTEM_KEY_PRESS = 28;
    private static final int MSG_HANDLE_ALL_APPS = 29;
    ...
    class SettingsObserver extends ContentObserver {
	    ...
	    resolver.registerContentObserver(Settings.System.getUriFor(
            ...
            resolver.registerContentObserver(Settings.Global.getUriFor(
                    ...
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.System.getUriFor(
                    Settings.System.LONG_PRESS_POWER_TORCH), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.System.getUriFor(
    ...
    private void handleDoubleTapOnHome() {
        ...
        switch (mDoubleTapOnHomeBehaviorCustom) {
            case DOUBLE_TAP_HOME_RECENT_SYSTEM_UI:
                ...
                break;
            case LONG_PRESS_HOME_ASSIST:
                launchAssistAction(null, mEventDeviceId);
                break;
            default:
        ...
        synchronized (mLock) {
           ...
           mSystemNavigationKeysEnabled = Settings.Secure.getIntForUser(resolver,
            ...
            mLongPressPowerTorch = Settings.System.getIntForUser(resolver,
                    Settings.System.LONG_PRESS_POWER_TORCH, 0,
                    UserHandle.USER_CURRENT) != 0;
    ...
    public void sendCustomAction(Intent intent) {
        ...
        if (action != null) {
            if (OmniUtils.INTENT_SCREENSHOT.equals(action)) {
                mContext.enforceCallingOrSelfPermission(Manifest.permission.ACCESS_SURFACE_FLINGER,
                        TAG + "sendCustomAction permission denied");
                mHandler.removeCallbacks(mScreenshotRunnable);
                mScreenshotRunnable.setScreenshotType(TAKE_SCREENSHOT_FULLSCREEN);
                mHandler.post(mScreenshotRunnable);
            } else if (OmniUtils.INTENT_REGION_SCREENSHOT.equals(action)) {
                mContext.enforceCallingOrSelfPermission(Manifest.permission.ACCESS_SURFACE_FLINGER,
                        TAG + "sendCustomAction permission denied");
                mHandler.removeCallbacks(mScreenshotRunnable);
